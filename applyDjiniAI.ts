...
function isAiMlRole(title: string, description: string): boolean {
  ...
  const isIncluded = includePatterns.some(re => re.test(haystack));
  const isExcluded = excludePatterns.some(re => re.test(haystack));
...
try {
  ...
  // 3. Navigate to filtered AI/ML job listings
  const keywords = [
    'Machine Learning',
    'ML Engineer',
    'AI Engineer',
  ...
  for (let pageNum = 1; pageNum <= 5; pageNum++) {
    ...
    for (const link of jobLinks) {
      ...
      // Scrape job title and description early to avoid applying to non-AI/ML roles
      let jobTitle = '';
      try {
        jobTitle = await page.$eval('h1', el => el.textContent?.trim() || '');
      } catch {
        jobTitle = '';
      }
      let jobDescription = '';
      ...
      if (!isAiMlRole(jobTitle, jobDescription)) {
        console.log(`--> SKIPPING (Not AI/ML focused): ${link}`);
        continue;
      }
      ...
      for (const applyButton of applyButtons) {
        try {
          ...
          console.warn('Apply button not clickable, trying next one.', e);
        }
      }
